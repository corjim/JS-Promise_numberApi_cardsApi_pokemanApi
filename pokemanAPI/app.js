...
// names and URLs for every pokemon in the database.
axios.get(`${baseURL}/pokemon/?limit=1000`).then(data => {
...
axios.get(`${baseURL}/pokemon/?limit=500`).then(data => {
    console.log(data);
    let randomPokemonUrls = [];
    for (let i = 0; i < 3; i++) {
        let randomIdx = Math.floor(Math.random() * data.data.results.length)
        console.log(randomIdx)
        let url = data.data.results.splice(randomIdx, 1)[0].url;
